...
open_image(void) {
	...
	while (modal_yield() && ufa_check_open_file(open_file) == UFA_PENDING) {
	}

	if (ufa_check_open_file(open_file) != UFA_OK) {
		goto end;
	}

	size_t file_size = ufa_get_open_file_size(open_file);
	void* buf = barena_memalign(&arena, file_size, 1);
	if (ufa_read_open_file(open_file, buf, &file_size) != UFA_OK) {
		BLOG_ERROR("Could not read file: %s", ufa_get_open_file_error(open_file));
		goto end;
	}

	CF_Image new_image = { };
	CF_Result result = cf_image_load_png_from_memory(buf, file_size, &new_image);
	if (result.code != CF_RESULT_SUCCESS) {
		BLOG_ERROR("Could decode image: %s", result.details);
		goto end;
	}
	...
	BLOG_INFO("Loaded %s", ufa_get_open_file_name(open_file));
...
save_slice(void) {
	...
	while (modal_yield() && ufa_check_save_file(save_file) == UFA_PENDING) {
	}

	if (ufa_check_save_file(save_file) != UFA_OK) {
		goto end;
	}
	...
	int png_len = 0;
	CF_Result result = cf_image_save_png_to_memory(&slice, &png, &png_len);
	if (result.code != CF_RESULT_SUCCESS) {
		BLOG_ERROR("Could not encode image: %s", result.details);
		goto end;
	}

	if (ufa_write_save_file(save_file, png, &(size_t){ png_len }) != UFA_OK) {
		BLOG_ERROR("Could not save image: %s", ufa_get_save_file_error(save_file));
		goto end;
	}

	BLOG_INFO("Saved %s", ufa_get_save_file_name(save_file));
...
update(void) {
	...
	switch (cmd) {
		case CMD_NONE:
			break;
		case CMD_LOAD:
			start_modal(open_image);
			break;
		case CMD_SAVE:
			start_modal(save_slice);
			break;
	}
